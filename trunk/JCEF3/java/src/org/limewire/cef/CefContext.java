...
public class CefContext {
	...
	public static final boolean initialize(String cachePath) {
		try {
			System.out.println("Initialize on thread_id "+Thread.currentThread().getId()+" "+Thread.currentThread().getName());
			System.loadLibrary("icudt");
			System.loadLibrary("libcef");
            System.loadLibrary("CefWrapper");
            N_Initialize(System.getProperty("sun.boot.library.path"), cachePath);
            return true;
