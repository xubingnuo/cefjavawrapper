...
void ClientApp::OnBeforeChildProcessLaunch(
      CefRefPtr<CefCommandLine> command_line) {
		  ...
		  CefString ss = command_line->GetCommandLineString();
    //command_line->InitFromString("java.exe");
	//ss = command_line->GetCommandLineString();
  ...
  g_command_line->InitFromArgv(argc, argv);
   ...
   CefString cmd = g_command_line->GetCommandLineString();
   printf_s("%s", cmd);
   std::string val = cmd.ToString();
    
    size_t pos_opt = val.find("-agentlib");
	std::string clean_str = val;
	if (pos_opt!=-1){
		size_t pos_end = val.find(' ', pos_opt);
		clean_str = val.substr(0, pos_opt) + val.substr(pos_end);
	}
	clean_str+=" stub";
	ReplaceStringInPlace( clean_str, "-d", "-D");	 		
	ReplaceStringInPlace( clean_str, "\\", "/");	 	
	ReplaceStringInPlace( clean_str, "\"", "");	 	
	printf("cmd %s", clean_str);	
	
    command_line->PrependWrapper(clean_str);   	
  /*  
  BrowserDelegateSet::iterator it = browser_delegates_.begin();
  for (; it != browser_delegates_.end(); ++it)
