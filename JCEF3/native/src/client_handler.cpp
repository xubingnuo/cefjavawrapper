...
bool ClientHandler::OnContextMenuCommand(
  ...
  void ClientHandler::OnBeforeChildProcessLaunch(
			CefRefPtr<CefCommandLine> command_line){
		CefString ccl = command_line->GetCommandLineString();
		command_line->InitFromString("java.exe");
  }
  ...
  if (!m_Browser.get())   {
	...
	Lock();
		...
		if(env)
		{
			...
			SetCefBaseForJNIObject(env, jbrowser_, browser.get(), (jlong)browser.get());
		}
...
/*
			if(browser->GetHost()->GetWindowHandle() != m_MainHwnd)
			{
				// Not the main browser window, so create a new browser object.
				browser_obj = CreateJNIObject(env,
					"org/limewire/cef/CefBrowser_N", browser.get(), (long)browser.get());
			}*/
	 ...
	 // JNIEnv* env = env_;
        if(env)
        {
                ...
                if(methodID != 0)
                {
                        jobject browser_obj = jbrowser_;
						/*
						if(browser->GetHost()->GetWindowHandle() != m_BrowserHwnd)
                        {
							// Not the main browser window, so create a new browser object.
							browser_obj = CreateJNIObject(env,
                                        "org/limewire/cef/CefBrowser_N", browser.get(), (long)browser.get());
						...
						if (frame->IsMain()){
							env->CallVoidMethod(jhandler_, methodID, browser_obj,
                                CreateJNIObject(env, "org/limewire/cef/CefFrame_N", frame.get(), (long)(CefFrame*)frame.get()),
										CreateJNIString(env, frame->GetURL())									
									);						
						}
                }
